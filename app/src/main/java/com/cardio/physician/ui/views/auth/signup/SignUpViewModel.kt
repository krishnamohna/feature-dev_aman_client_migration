...
class SignUpViewModel @Inject constructor(
    ...
    //for saving fragment state when coming back from next fragmentm
    val mutableFragmentState=MutableLiveData<Bundle>()
    val validationChannel = Channel<ValidationModel>(ENUM.INT_10)
    var deviceUri: Uri? = null

    fun getLiveDataFragmentState(): LiveData<Bundle> = mutableFragmentState

    fun validateFieldsToSetAlpha(
        isChecked: Boolean, firstName: String,lastname:String, email: String,
        ...
        when {
            ...
            lastname.isEmpty() || !lastname.validUserNameLength() -> {
                setObserverForAlpha(R.string.alpha_false)
            }
            email.isEmpty() -> {
                setObserverForAlpha(R.string.alpha_false)
    ...
    suspend fun validateFields(
        ...
        password: String, confirmPassword: String, onValidationSuccess:(countryCode:String,phoneNo:String,email:String)->Unit
    ) {
        ...
        * */
        ...
        if (isValidFirstName && isValidEmail && isValidPassword && isValidConfirmPassword && isValidPhoneNumber && isValidLastName) {
            onValidationSuccess.invoke(countryCode,phoneNumber,email)
        }
    }

    fun checkIsUserExist(countryCode: String, phoneNumber: String, email: String) {
        try {
