...
import com.cardio.physician.ui.common.utils.Constants.HEIGHT_DIGITS_BEFORE_ZERO
...
@AndroidEntryPoint
class EditProfileFragment : BaseToolBarFragment<FragmentEditProfileBinding>(),
    ...
    private fun setViews() {
        binding.edtWeight.filters = arrayOf<InputFilter>(DecimalDigitsInputFilter(WEIGHT_DIGITS_BEFORE_ZERO, 2))
        binding.edtHeight.filters = arrayOf<InputFilter>(DecimalDigitsInputFilter(HEIGHT_DIGITS_BEFORE_ZERO, 2))
        binding.spinnerCategory.adapter = ArrayAdapter<String>(requireContext(),
...
//                            .into(binding.imgProfilePic)
                    ...
                    Constants.USER_GENDER -> {
                        ...
                        binding.txtTitleGender.visibility =
                            if (gender.isNullOrBlank() || gender.equals(getString(R.string.select_gender),
                                    true)
                            ) {
                                GONE
                            } else View.VISIBLE
    ...
    private fun showUserDetailOnUI(documentReference: DocumentSnapshot?) {
        if (documentReference != null && documentReference.data != null) {
            ...
            val dob = documentReference.data?.get(FireStoreDocKey.DOB_PROFILE) as String?
            ...
            try {
                if (!dob.isNullOrEmpty()) birthDate =
                    dob.datePickerStringToDate(DATE_FORMAT_DD_MMM_YYYY)
            } catch (ex: Exception) {
            ...
            userType?.let {
                binding.phoneNumberValContainer.visibility = if (it == SignUpUserType.GOOGLE.name) {
                    View.GONE
                } else {
                    View.VISIBLE
                }
    ...
    private fun setViewVisiblitiesForData(extrasUserModel: UserModel?) {
        extrasUserModel?.run {
            ...
            height?.isNotEmpty()?.let { if (it) binding.txtTitleHeight.visibility = View.VISIBLE }
            weight?.isNotEmpty()?.let { if (it) binding.txtTitleWeight.visibility = View.VISIBLE }
        }
    }

    override fun getToolbarImp(): IToolbar? =
        EditProfileToolBarImp(binding.headerView.toolBarContainer).registerBackButtonListener {
            onBackButtonClick()
        }
