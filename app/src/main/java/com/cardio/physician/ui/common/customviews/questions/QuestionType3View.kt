...
class QuestionType3View @JvmOverloads constructor(
    ...
    private fun setSelection() {
        question.answer?.let {
            for (i in 0 until binding.radioGroupQuestion.childCount) {
                var radioButton = binding.radioGroupQuestion.getChildAt(i) as RadioButton
                if (it.equals(radioButton.text.toString(), true)) {
                    radioButton.isChecked = true
                    if (radioButton.id == R.id.rbOption1) {
                        showSecondaryOptions(true)
                        setSelectionOfSecondaryQuestion()
                    }
                    break
                }
            }
    ...
    private fun setSelectionOfSecondaryQuestion() {
        question.answerSecondary?.let {
            for (j in 0 until binding.rbGroupLeads.childCount) {
