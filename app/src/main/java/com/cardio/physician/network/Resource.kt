...
data class Resource<out T>(
    ...
    companion object {
        ...
        fun <T> error(apiConstant: String, errorCode: Int, msg: String, data: T?,exception: Exception?=null): Resource<T> {
            //we can send our messages by using errorCode
            when (errorCode) {
                404 -> {
                    //val message = "Detail Not found"
                    return Resource(apiConstant, errorCode, Status.ERROR, data, msg, 0,exception)
                }
                in 400..499 -> {
                ...
                else -> return Resource(apiConstant, errorCode, Status.ERROR, data, msg, 0,exception)
            }
        }
